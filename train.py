...
def align_depth(depth_render, depth_vggt):
    '''
    min(\|l * depth_vggt + \mu - depth_render\|^2)
    depth_vggt: N, 1
    depth_render: N, 1
    '''
    mean_render = depth_render.mean()
    depth_render_residual = depth_render - mean_render
    mean_vggt = depth_vggt.mean()
    depth_vggt_residual = depth_vggt - mean_vggt
    l = (depth_vggt_residual * depth_render_residual).sum() / (depth_vggt_residual * depth_vggt_residual).sum()
    mu = mean_render - l * mean_vggt
    return l, mu
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        gt_image, gt_image_gray, res = viewpoint_cam.get_image()
        loaded_mask = res["mask"]
        depth_vggt = res["depth_vggt"]
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, bg, app_model=app_model,
                            return_plane=iteration>3000, return_depth_normal=iteration>3000)
        ...
        if loaded_mask is not None and args.mask_background and iteration > opt.single_view_weight_from_iter:
            loss += 0.1 * (render_pkg["rendered_alpha"].squeeze())[loaded_mask.squeeze() < 0.5].mean()

        # VGGT
        if iteration > 3000 and depth_vggt is not None and args.use_depth and iteration < opt.single_view_weight_from_iter:
            image_weight = (1.0 - get_img_grad_weight(gt_image))
            image_weight = (image_weight).clamp(0,1).detach() ** 4
            rendered_depth = render_pkg['plane_depth'].squeeze()

            mask = (loaded_mask.float().reshape(-1) > 0.5) #& (viewpoint_cam.get_depth_conf_vggt().reshape(-1) > 0.5)
           
            l, mu = align_depth(rendered_depth.reshape(-1)[mask].detach(), (depth_vggt).reshape(-1)[mask])
           
            depth_vggt = l * (depth_vggt) + mu
            
            depth_diff = (image_weight * (rendered_depth - depth_vggt)).abs().reshape(-1)
            depth_weight = args.vfm_weight #0.1
            if args.mask_type == "dynamic":
                conf_weight = viewpoint_cam.get_depth_conf_vggt()**((iteration - 3000)/3000)
            else:
                conf_weight = torch.ones_like(viewpoint_cam.get_depth_conf_vggt()) * loaded_mask.squeeze()
            depth_loss = depth_weight * (depth_diff * conf_weight.reshape(-1)).sum() / ((loaded_mask.squeeze().float() > 0).sum() + 1e-8)
            loss += depth_loss# + 0.05 * (- torch.log(viewpoint_cam.get_depth_conf_vggt()[loaded_mask.squeeze().float() > 0])).mean()

            if True:
                normal_vggt = render_normal(viewpoint_cam, depth_vggt.squeeze().detach())
                normal = render_pkg["rendered_normal"] 
                # mask = res["mask"].float().reshape(-1) > 0
                normal_loss = (image_weight * (normal - normal_vggt.detach()).abs().sum(0) * conf_weight.squeeze()).reshape(-1)

                depth_normal = render_pkg["depth_normal"]
                depth_normal_loss = (image_weight * (depth_normal - normal_vggt * render_pkg["rendered_alpha"].detach()).abs().sum(0) * conf_weight.squeeze()).reshape(-1)
                if loaded_mask is not None:
                    normal_loss = normal_loss[loaded_mask.float().reshape(-1) > 0].mean()
                    depth_normal_loss = depth_normal_loss[loaded_mask.float().reshape(-1) > 0].mean()
                    loss += normal_loss * args.vfm_weight + depth_normal_loss * args.vfm_weight
                else:
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                if iteration % 200 == 0:
                    ...
                    if viewpoint_cam.get_depth_conf_vggt() is not None:
                        depth_conf_show = (viewpoint_cam.get_depth_conf_vggt().squeeze().detach().cpu().numpy() * 255).astype(np.uint8)
                        depth_conf_color = cv2.applyColorMap(depth_conf_show, cv2.COLORMAP_BONE)
                        row0 = np.concatenate([row0, depth_conf_color], axis=1)
                        row1 = np.concatenate([row1, depth_conf_color], axis=1)
                    if depth_vggt is not None:
                        depth_vggt_b = (res["depth_vggt"] - res["depth_vggt"].min()) / (res["depth_vggt"].max() - res["depth_vggt"].min() + 1e-20)
                        depth_vggt_color = (depth_vggt_b.repeat((3, 1, 1)).permute(1, 2, 0).detach().cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
                        depth_vggt_a = (depth_vggt - depth_vggt.min()) / (depth_vggt.max() - depth_vggt.min() + 1e-20)
                        depth_vggt_color_after = (depth_vggt_a.repeat((3, 1, 1)).permute(1, 2, 0).detach().cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
                        row0 = np.concatenate([row0, depth_vggt_color_after], axis=1)
                        row1 = np.concatenate([row1, depth_vggt_color], axis=1)
                ...
                if d_mask.sum() > 0:
                    ...
                    if use_virtul_cam is False:
                        ...
                        if mask.sum() > 0:
                            ncc_loss = ncc_weight * ncc.mean() * args.ncc_weight
                            loss += ncc_loss
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                if viewpoint_cam.optimizer is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mask_type", type=str, default="none", choices=['none', 'conf', 'translab', 'image', 'depth', 'hybrid', 'dynamic'])
    parser.add_argument("--normal_type", type=str, default="none", choices=['none', 'vggt', 'sn'])
    parser.add_argument("--depth_type", type=str, default="none", choices=['none', 'DA', 'vggt'])
    parser.add_argument("--anchor_type", type=str, default="rn", choices=["rn", "hybrid"])
    parser.add_argument("--use_depth", action='store_true', default=False)
    parser.add_argument("--ncc_weight", type=float, default=1.0)
    parser.add_argument("--vfm_weight", type=float, default=0.01)
